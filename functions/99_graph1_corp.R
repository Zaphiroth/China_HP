...
graph1 <-
  function(salesdata,
    ...
    if (!is.null(level) &&
        level %in% c("corporation", "molecule")) {
    ...
    salesdata2 <- salesdata2 %>%
      group_by(AUDIT.DESC, CORPORATE.DESC, PRODUCT.DESC, date) %>%
      mutate(prod.sum = sum(value, na.rm = TRUE)) %>%
      ungroup() %>%
    ...
    salesdata2 <- salesdata2 %>%
      arrange(AUDIT.DESC, CORPORATE.DESC, PRODUCT.DESC, date)
    ...
    #                            FUN = sum.nona))
    salesdata3 <- salesdata3 %>%
      group_by(AUDIT.DESC,  date) %>%
      mutate(area.sum = sum(value, na.rm = TRUE)) %>%
      ungroup() %>%
      as.data.frame(., stringsAsFactors = FALSE)
    ...
    if ("pi" %in% kpi) {
      ...
      salesdata3 <- salesdata3 %>%
        group_by(AUDIT.DESC, date) %>%
        mutate(sum.market = sum(value, na.rm = TRUE)) %>%
        ungroup()
    ...
    salesdata3$MARKET.DESC <- "Overall"
    salesdata3$MANUF.TYPE.DESC <- "Overall"
    salesdata3$TC.I.SHORT.DESC <- "Overall"
    salesdata3$TC.I.DESC <- "Overall"
    salesdata3$TC.II.SHORT.DESC <- "Overall"
    salesdata3$TC.II.DESC <- "Overall"
    salesdata3$TC.III.SHORT.DESC <- "Overall"
    salesdata3$TC.III.DESC <- "Overall"
    salesdata3$CORPORATE.DESC <- "Overall"
    salesdata3$CORP.CN <- "Overall"
    salesdata3$PRD.CODE <- "Overall"
    salesdata3$PRODUCT.DESC <- "Overall"
    salesdata3$PRD.CN <- "Overall"
    salesdata3$COMPS.ABBR <- "Overall"
    salesdata3$COMPS.DESC <- "Overall"
    salesdata3$COMPS.CN <- "Overall"
    ...
    if (!is.null(level) &&
      ...
      if (level == "corporation") {
        ...
        salesdata4 <- salesdata4 %>%
          group_by(AUDIT.DESC, CORPORATE.DESC, date) %>%
          mutate(prod.sum = sum(value, na.rm = TRUE)) %>%
          ungroup() %>%
          as.data.frame(., stringsAsFactors = FALSE)
        ...
        if ("gr" %in% kpi) {
          salesdata4 <-
            transform(salesdata4, gr = ave(value,
                                           paste(
                                             AUDIT.DESC,
                                             CORPORATE.DESC,
                                             # PRODUCT.DESC,
        ...
        if ("mc" %in% kpi) {
          ...
          salesdata4 <-
            transform(salesdata4, mc = ave(ms,
                                           paste(
                                             AUDIT.DESC,
                                             CORPORATE.DESC,
                                             # PRODUCT.DESC,
          ...
          chinaref <-
            salesdata4[which(salesdata4$AUDIT.DESC == "China"),
                       c(#"CORPORATE.DESC",
                         ...
                         "value")]
          chinaref <-
            plyr::rename(chinaref, c("value" = "cn.value"))
          salesdata4 <-
            join(salesdata4,
                 chinaref,
                 by = c(#"CORPORATE.DESC",
        ...
        salesdata4$MARKET.DESC <- "Overall"
        salesdata4$MANUF.TYPE.DESC <- "Overall"
        salesdata4$TC.I.SHORT.DESC <- "Overall"
        salesdata4$TC.I.DESC <- "Overall"
        salesdata4$TC.II.SHORT.DESC <- "Overall"
        salesdata4$TC.II.DESC <- "Overall"
        salesdata4$TC.III.SHORT.DESC <- "Overall"
        salesdata4$TC.III.DESC <- "Overall"
        # salesdata3$CORPORATE.DESC <- "Overall"
        # salesdata3$CORP.CN <- "Overall"
        salesdata4$PRD.CODE <- "Overall"
        salesdata4$PRODUCT.DESC <- "Overall"
        salesdata4$PRD.CN <- "Overall"
        salesdata4$COMPS.ABBR <- "Overall"
        salesdata4$COMPS.DESC <- "Overall"
        salesdata4$COMPS.CN <- "Overall"
        ...
        salesdata4 <- salesdata4 %>%
          group_by(AUDIT.DESC,  COMPS.DESC, date) %>%
          mutate(prod.sum = sum(value, na.rm = TRUE)) %>%
          ungroup() %>%
          as.data.frame(., stringsAsFactors = FALSE)
    ...
    final <-
      data.table::dcast(
        setDT(final),
        AUDIT.DESC + MARKET.DESC + MANUF.TYPE.DESC + TC.I.SHORT.DESC +
          TC.I.DESC + TC.II.SHORT.DESC + TC.II.DESC + TC.III.SHORT.DESC +
          TC.III.DESC +
          CORPORATE.DESC + CORP.CN + PRD.CODE + PRODUCT.DESC + PRD.CN +
          COMPS.ABBR + COMPS.DESC + COMPS.CN +
          Measure + Index
        ~ date,
        value.var = "value"
      )
